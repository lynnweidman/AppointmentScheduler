...
public class UpdateAnAppointmentController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            ...
            appointmentsColumn.setCellValueFactory(new PropertyValueFactory<>("appointmentID"));
       ...
       AddAppointmentsModel selectedAppointment = (AddAppointmentsModel) appointmentsTable.getSelectionModel().getSelectedItem();
        int firstLevelDivisionId = FirstLevelDivisionsUtil.getTheDivisionByCustomerId(selectedAppointment.getCustomerID());
            ...
            customerNameTxt.setText(String.valueOf(NamesUtil.getCustomerName(selectedAppointment.getAppointmentID())));
            userNameTXT.setText(String.valueOf(NamesUtil.getUserName(selectedAppointment.getAppointmentID())));
    ...
    public void updateAppointmentButtonPressed(ActionEvent actionEvent) throws SQLException {
        ...
        for (AddAppointmentsModel apps : AddAppointmentsUtil.allAppointments) {
            ...
            int getDatabaseId = apps.getAppointmentID();
        ...
        if (this.endDatePicker.equals(null)) {
            Alert invalidTime = new Alert(Alert.AlertType.ERROR);
            invalidTime.setContentText("Must choose End Date,Hour and Minutes  to update an appointment.");
            invalidTime.showAndWait();
            return;
        ...
        try {
            ...
            Timestamp lastUpdate = Timestamp.valueOf(LocalDateTime.now(ZoneOffset.UTC).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
            System.out.println(userNameTXT.getText());
            String lastUpdatedBy = userNameTXT.getText();
            int customerID = Integer.parseInt(customerIdTXT.getText());
            int userID = Integer.parseInt(userIdTXT.getText());
            int contactID = contactComboBox.getSelectionModel().getSelectedItem().getContactID();
            String customerType = customerTypeTxt.getText();

            UpdateAppointmentUtil.updateAppointmentsTable(appointmentID, title, description,
                    location, type, start, end, lastUpdate, lastUpdatedBy, customerID, userID, contactID);

            appointmentsTable.setItems(AddAppointmentsUtil.getAllAppointmentsLDT());

            //Adding to the TypeCustomerModel- polymorphic example.
            TypeCustomerModel customersWithType = new TypeCustomerModel(appointmentID, title, description, location, type, start.toLocalDateTime(), end.toLocalDateTime(), lastUpdate.toLocalDateTime(), lastUpdatedBy, customerID, userID, contactID, customerType);
